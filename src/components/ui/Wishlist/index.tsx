...
const wishlistStyles = cva('w-12 h-12 flex items-center justify-center rounded-full', {
  variants: {
    selected: {
      ...
      false: "bg-grayscale-200 hover:bg-pink-200 [&_path]:fill-transparent [&_path]:hover:stroke-red-500"
    }
  }
})

interface IWishlist extends React.ButtonHTMLAttributes<HTMLButtonElement>,
  VariantProps<typeof wishlistStyles> {
  selected: boolean
}

const Wishlist: React.FC<IWishlist> = ({ selected, ...props }) => {
  return (
    <button {...props} className={cx(wishlistStyles({ selected }), "")}>
